TARGETNAME := obj\main\cleanflightF4_COLIBRI.elf
...
VALID_TARGETS	 += ANYFC REVO COLIBRI STM32F4DISCOVERY
...
FLASH_SIZE = 256
else ifeq ($(TARGET),$(filter $(TARGET),STM32F4DISCOVERY))
FLASH_SIZE = 1024
else
...
else ifeq ($(TARGET),$(filter $(TARGET),ANYFC REVO COLIBRI STM32F4DISCOVERY))
...
DEVICE_FLAGS += -DHSE_VALUE=16000000
LD_SCRIPT	 = $(LINKER_DIR)/stm32_flash_f407_$(FLASH_SIZE)k.ld
.DEFAULT_GOAL := binary
endif
TARGET_FLAGS = -D$(TARGET)
...
STM32F4DISCOVERY_SRC = startup_stm32f40xx.s \
